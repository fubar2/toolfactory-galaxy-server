...
for galaxy_dir in "${!exports[@]}"; do
  ...
  if ! [[ "$galaxy_dir" == "$GALAXY_VIRTUAL_ENV" ]] ; then
    rm -rf $galaxy_dir
...
"$GALAXY_VIRTUAL_ENV/bin/uwsgi" --yaml "$GALAXY_CONFIG_DIR/galaxy.yml" --uid "$GALAXY_UID" --gid "$GALAXY_GID" --touch-reload "/galaxy/config/reload_uwsgi.touchme"
